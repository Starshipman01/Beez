...
const createNewDelivery = async(order) => {
  console.log("ADDRESS USED Delivery: ", expressKeys.createNewDeliveryURL)
  
  try{ 
    const { token, ...orderWithoutToken } = order;
    console.log("TOKEN in appwrite createNewDelivery:", token)
    const response = await fetch(expressKeys.createNewDeliveryURL, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`,
      },
      body: JSON.stringify(orderWithoutToken),
    });
    const data = await response.json();
    console.log("data: ", data)
    if (!response.ok) {
